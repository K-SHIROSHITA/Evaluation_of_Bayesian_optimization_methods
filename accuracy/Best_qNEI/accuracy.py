...
def main(n, m, dim, noise, bounds):
    ...
    image_name = "./Best_qNEI/image/val_2D"
    plot_6d(points, val_y, image_name=image_name)

    # 予測平均をplot(2次元だけ取り出してあとは平均値)
    image_name = "./Best_qNEI/image/pred_y_mean_2D_0"
    ...
    for i in range(10):
        ...
        next_X1 = observation_max_points(model=model, results=train_X.float())

        next_X = torch.cat((next_X0.reshape(-1, 6), next_X1.reshape(-1, 6)), dim=0).type(torch.float64)
        ...
        # 予測平均をplot(2次元だけ取り出してあとは平均値)
        image_name = "./Best_qNEI/image/pred_y_mean_2D_" + str(i + 1)
        plot_6d(points, pred_y, image_name=image_name)
    ...
    plt.savefig("./Best_qNEI/image/mse.png")
